...
class ImageViewer(object):
    ...
    * 'ESC' : quit
    ...
    def line(self, points_list, label=None):
        if len(points_list) <= 1:
            return
        for index in range(1, len(points_list)):
            cv2.line(self.image,(points_list[index-1][0], points_list[index-1][1]),
                (points_list[index][0], points_list[index][1]),self._color,self.thickness)
        
        pt1 = points_list[0][0], points_list[0][1]
        if label is not None:
            text_size = cv2.getTextSize(
                label, cv2.FONT_HERSHEY_PLAIN, 1, self.thickness)
    ...
    def do_write( self ):
        if (self._video_writer is not None) and (self.image.shape[0] != 0):
